...
def attach_attention_module(net, attention_module, block_scope=None):
  if attention_module == 'se_block': # SE_block
    ...
    net = se_block_v2(net, se_block_scope)
...
def se_block_v2(input_feature, name, ratio=8):
    ...
    with tf.variable_scope(name):
        ...
        # Global average pooling
        squeeze = tf.reduce_mean(input_feature, axis=[1, 2], keepdims=True)
        squeeze = tf.cast(squeeze, tf.float32)
        nfeatures = int(channel // ratio)
        print('input_feature:',input_feature)
        excitation = slim.fully_connected(squeeze,
                                          int(channel // ratio),
                                          weights_initializer=trunc_normal(0.02),
