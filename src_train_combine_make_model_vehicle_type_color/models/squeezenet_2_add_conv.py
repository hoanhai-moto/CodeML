    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        with tf.variable_scope('squeezenet', [images], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout],
                ...
                net = slim.conv2d(images, 256, [7, 7], stride=2, scope='conv4')
                print(net)
                net = se_block(net, "se_block_9", ratio=8)
                print(net)

                net = slim.conv2d(net, 128, [7, 7], stride=2, scope='conv6')
                print(net)
                net = se_block(net, "se_block_6", ratio=8)
                print(net)

                net = slim.conv2d(net, 96, [7, 7], stride=2, scope='conv7')
                print(net)
                net = slim.max_pool2d(net, [3, 3], stride=2, scope='maxpool7')
                print(net)
                net = se_block(net, "se_block_7", ratio=8)
                print(net)
