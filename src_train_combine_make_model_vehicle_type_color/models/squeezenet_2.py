    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        with tf.variable_scope('squeezenet', [images], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout],
                ...
                net = slim.conv2d(images, 96, [3, 3], stride=2, scope='conv1')
                print(net)
                ...
                fire2 = fire_module(net, 48, 128, scope='fire2')
                # print(net)
                fire3 = fire_module(fire2, 48, 128, scope='fire3')
                # print(net)
                ...
                fire4 = fire_module(net, 48, 128, scope='fire4')
                # print(net)
                fire5 = fire_module(fire4, 48, 128, scope='fire5')
                # print(net)
                ...
                fire6 = fire_module(net, 64, 192, scope='fire6')
                # print(net)
                fire7 = fire_module(fire6, 64, 192, scope='fire7')
                # print(net)
...
def inference_head(images, keep_probability, phase_train=True, bottleneck_layer_size=128, weight_decay=0.0, reuse=None):
    batch_norm_params = {
        # Decay for the moving averages.
        'decay': 0.995,
        # epsilon to prevent 0s in variance.
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        with tf.variable_scope('squeezenet', [images], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout],
                ...
                # fire4 = fire_module(net, 32, 128, scope='fire4')
                # # print(net)
                ...
                # fire6 = fire_module(net, 48, 192, scope='fire6')
                # # print(net)
...
def inference_tail(images, keep_probability, phase_train=True, bottleneck_layer_size=128, weight_decay=0.0, reuse=None):
    batch_norm_params = {
        # Decay for the moving averages.
        'decay': 0.995,
        # epsilon to prevent 0s in variance.
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                        ...
                        normalizer_fn=slim.batch_norm,
                        normalizer_params=batch_norm_params):
        with tf.variable_scope('squz_tail_color', [images], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout],
                                is_training=phase_train):
                # net = slim.conv2d(images, 4, [3, 3], stride=1, scope='conv0')
                ...
                fire2 = fire_module(images, 32, 128, scope='fire2')
                # print(net)
                fire3 = fire_module(fire2, 32, 128, scope='fire3')
                # print(net)
                net = tf.add(fire3, fire2, name="bypass23")
