...
def conv2d_same(inputs, num_outputs, kernel_size, stride, rate=1, scope=None):
    if stride == 1:
        ...
        return slim.conv2d(
            ...
            rate=rate, padding='VALID', scope=scope, activation_fn=tf.nn.swish
        )
...
def block(inputs,
    ...
    net = slim.conv2d(
        ...
        padding='SAME', scope=name + 'project_conv', activation_fn=tf.nn.swish
    )
...
def EfficientNet(
    ...
    net = slim.conv2d(
        ...
        padding='SAME', scope="top_convd", activation_fn=tf.nn.swish
    )

    return net


def EfficientNetB4(images, bottleneck_layer_size, phase_train, weight_decay, reuse=None,keep_probability=0.7):
    batch_norm_params = {
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        with tf.variable_scope('EfficientNetB4', [images], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=phase_train):
                ...
                net = slim.dropout(net, keep_prob=keep_probability, is_training=phase_train, scope='dropout_fn')
...
def EfficientNetB7(images, bottleneck_layer_size, phase_train, weight_decay, reuse=None,keep_probability=0.7):
    batch_norm_params = {
        # Decay for the moving averages.
        'decay': 0.995,
        # epsilon to prevent 0s in variance.
