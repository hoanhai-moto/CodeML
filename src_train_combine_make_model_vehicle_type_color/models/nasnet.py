...
def view_pool(view_features, name):
    ...
    for v in view_features[1:]:
        ...
        vp = tf.concat([vp, v], 0)
    print('Viewpool before reducing:', vp.get_shape().as_list())
...
def _imagenet_stem(inputs, hparams, stem_cell):
    ...
    for i in range(n_views):
        view = tf.gather(inputs, i)
        with tf.variable_scope("MultiView", reuse=True if i > 0 else None):
            tf.summary.image('MultiView', view, 1)
            # 149 x 149 x 32
            num_stem_filters = int(32 * hparams.stem_multiplier)
            net = slim.conv2d(
                view, num_stem_filters, [3, 3], stride=2, scope='conv0',
                padding='VALID')
            net = slim.batch_norm(net, scope='conv0_bn')
            view_features.append(net)

    net = view_pool(view_features, 'View_pool')
