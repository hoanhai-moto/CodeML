    ...
    with tf.variable_scope(scope, 'InceptionResnetV1', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
                ...
                net = slim.avg_pool2d(net, 3, stride=2, padding='VALID', scope='MaxPool_1a_3x3') ###########
                ...
                #net = slim.avg_pool2d(net, 3, stride=2, padding='VALID', scope='MaxPool_4a_3x3') ##########
...
def inference_head(images, keep_probability, phase_train=True, 
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                        ...
                        normalizer_fn=slim.batch_norm,
                        normalizer_params=batch_norm_params):
        return inception_resnet_v1_head(images,
                                   is_training=phase_train,
                                   dropout_keep_prob=keep_probability,
                                   bottleneck_layer_size=bottleneck_layer_size,
                                   reuse=reuse)
...
def inference_tail(images, keep_probability, phase_train=True, 
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                        ...
                        normalizer_fn=slim.batch_norm,
                        normalizer_params=batch_norm_params):
        return inception_resnet_v1_tail(images,
                                   is_training=phase_train,
                                   dropout_keep_prob=keep_probability,
                                   bottleneck_layer_size=bottleneck_layer_size,
                                   reuse=reuse)
    ...
    with tf.variable_scope(scope, 'InceptionResnetV1', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
                ...
                # 5 x Inception-resnet-A
                net = slim.repeat(inputs, 1, block35, scale=0.17)
                # #net = slim.repeat(net, 10, block35, scale=0.17)
                # end_points['Mixed_5a'] = net
