    ...
    with tf.variable_scope(scope, 'InceptionResnetV1', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
                ...
                # net = slim.conv2d(net, 128, 3, padding='VALID', scope='Conv2d_4a_3x3')
                net = slim.conv2d(net, 128, 3, stride=2, padding='VALID', scope='Conv2d_4b_3x3')

                net = slim.repeat(net, 2, block35, scale=0.5)

                with tf.variable_scope('Mixed_6a'):
                    net = reduction_a(net, 128, 128, 128, 128)

                net = slim.repeat(net, 2, block17, scale=0.5)

                with tf.variable_scope('Mixed_7a'):
                    net = reduction_a(net, 256, 256, 384, 384)
                    # net = reduction_b(net)

                net = slim.repeat(net, 2, block8, scale=0.5)
...
def inference_vehicle_type(inputs,
                           keep_probability,
                           phase_train=True,
                           bottleneck_layer_size=128,
                           weight_decay=0.0,
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        with tf.variable_scope(scope, 'vehicle_type', [inputs], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout],
